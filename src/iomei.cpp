...
bool MEIInput::ReadSectionChildren(Object *parent, pugi::xml_node parentNode)
{
    ...
    for (current = parentNode.first_child(); current; current = current.next_sibling()) {
        ...
        // measure-level control elements exported as siblings of <staff> in neon-neume-line sections
            ...
            // neon-neume-line score-based MEI exports control elements as siblings
            // of <staff> under <section>, same shape as <slur>.
            if (!unmeasured) {
                if (parent->Is(SECTION)) {
                    if (m_doc->IsNeumeLines()) {
                        unmeasured = new Measure(NEUMELINE);
